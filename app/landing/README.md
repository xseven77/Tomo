...
```bash
./bin/dk release tomo   # 在 qiizo-docker-tools 目录
qiizo-deploy tomo
```

## SEO

部署前复制 `.env.example` 为 `.env.local`（本地）或 `${QIIZO_DATA}/tomo/.env`（生产构建）：

```bash
NEXT_PUBLIC_SITE_URL=https://tomo.qiizo.cn
```
