...
echo "==> 使用 Keychain 中 Tomo OAuth token 探测 ChatGPT API（需已登录）"
swift build -c release >/dev/null
BIN="${ROOT_DIR}/.build/release/Tomo"
if [[ ! -x "${BIN}" ]]; then
...
LATEST="${HOME}/Library/Application Support/Tomo/api-probes/latest/manifest.json"
if [[ -f "${LATEST}" ]]; then
  echo ""
